...
func _process(_delta):
	if not nene_rescued and player_inside_range and Input.is_action_just_pressed("action"):
		...
		for object in SharedVariables.player.objects_in_hand:
			if (object == object_keyword_neccesary and object_keyword_neccesary != "machete") or object_keyword_neccesary == "" :
				print("you passed")
				...
				nene_rescued = true
				return
			elif object == "machete" and object_keyword_neccesary == "machete":
				var tileset = get_parent().get_parent().get_node("Walls")
